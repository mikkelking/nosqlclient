...
Connection.prototype = {
  ...
  async migrateConnectionsIfExist() {
    ...
    const records = await Database.read({ type: Database.types.Connections });
    records.forEach((oldConnection) => {
      ...
      if (oldConnection.name) {
        ...
        if (oldConnection.user && oldConnection.password) {
          ...
          if (oldConnection.authDatabaseName)
            connection.scram_sha_1.authSource = oldConnection.authDatabaseName;
          connection.authenticationType = "scram_sha_1";
        }
  ...
  async tryInjectDefaultConnection() {
