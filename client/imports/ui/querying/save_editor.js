...
Editor.prototype = {
  getChangedObjects(
    ...
    findData.forEach((oldObj) => {
      const currentObj = _.find(activeEditorValue, (item) =>
        _.isEqual(item._id, oldObj._id)
      )
    ...
    activeEditorValue.forEach((currentObj) => {
      const foundObj = _.find(findData, (oldObj) =>
        _.isEqual(currentObj._id, oldObj._id)
      )
  ...
  saveFindEditor() {
    ...
    const activeEditorValue = ExtendedJSON.convertAndCheckJSON(
      QueryRender.getActiveEditorValue()
    )
    if (activeEditorValue.ERROR) {
      Notification.error("syntax-error-result", null, {
        error: activeEditorValue.ERROR,
      })
      return
    }
    ...
    Notification.modal({
      ...
      textTranslateOptions: {
        deleted: deletedObjectIds.length,
        updated: updateObjects.length,
        inserted: addedObjects.length,
        selectedCollection,
      },
      type: "info",
      callback: (isConfirm) => {
  ...
  saveEditor() {
    const doc = ExtendedJSON.convertAndCheckJSON(
      QueryRender.getActiveEditorValue()
    )
    if (doc.ERROR) {
