...
const isBuiltinRole = function () {
  if (
    SessionManager.get(SessionManager.strSessionUsermanagementRole) &&
    SessionManager.get(SessionManager.strSessionUsermanagementRole).isBuiltin &&
    $("#addEditRoleModalTitle").text() ===
      Helper.translate({ key: "edit_role" })
  ) {
...
const getActionColumn = function ({
  isBuiltin = false,
  actionType = "DELETE",
  targetIndex = 2,
  editClass = "editor_edit_privilege",
  deleteClass = "editor_delete",
}) {
  return {
    targets: [targetIndex],
    data: null,
    ...
    render(data, type, full) {
      if (isBuiltin || full.isBuiltin) {
        if (actionType === "VIEW")
          return `<a href="" title="View" class="${editClass}"><i class="fa fa-eye text-navy"></i></a>`
        return '<a href="" title="Not Allowed"><i class="fa fa-ban text-navy"></i></a>'
      }
      if (actionType === "DELETE")
        return `<a href="" title="Delete" class="${deleteClass}"><i class="fa fa-remove text-navy"></i></a>`
      return `<a href="" title="Edit" class="${editClass}"><i class="fa fa-edit text-navy"></i></a>`
    },
  }
}

const initCollectionsCombobox = function (
  collectionToSelect,
  selector,
  stopLadda,
  data = {}
  ...
  if (collectionToSelect)
    selector.val(collectionToSelect).trigger("chosen:updated")
...
UserManagementRoles.prototype = {
  addInheritRole() {
    ...
    if (table.rows().data().length === 0)
      this.populateRolesToInheritTable(null, [objectToAdd])
    else table.row.add(objectToAdd).draw()

    Notification.success("added-successfully", null, {
      fieldName: `<b>${role}</b>@${db}`,
    })
  },
  ...
  applyPrivilegeToRole() {
    ...
    if (
      $("#addEditPrivilegeModalTitle").text() ===
      Helper.translate({ key: "edit_privilege" })
    ) {
      // edit existing privilege of role
      const selectedRowData = SessionManager.get(
        SessionManager.strSessionUsermanagementPrivilege
      )

      privilegesTableSelector.rows().every(function () {
        const privilegesData = this.data()
        if (
          _.isEqual(privilegesData.privilege, selectedRowData.privilege) &&
      ...
      if (privilegesTableSelector.rows().data().length === 0)
        this.populateRolePrivilegesTable(null, [objectToAdd])
  ...
  addNewPrivilegeToRole() {
    ...
    $("#addEditPrivilegeModalTitle").text(
      Helper.translate({ key: "add_privilege" })
    )
    $("#addEditPrivilegeModalText").text(
      `${
        SessionManager.get(SessionManager.strSessionUsermanagementRole)
          ? SessionManager.get(SessionManager.strSessionUsermanagementRole).role
          : ""
      }`
    )
  ...
  startEditingRole() {
    if (!SessionManager.get(SessionManager.strSessionUsermanagementPrivilege))
      return
    ...
    const selectedResource = SessionManager.get(
      SessionManager.strSessionUsermanagementPrivilege
    ).resource
    let dbToSelect = ""
    ...
    if (
      ...
      if (selectedResource.indexOf("@") !== -1) {
        ...
        collectionToSelect = selectedResource
          .substr(0, selectedResource.indexOf("@"))
          .replace("<b>", "")
          .replace("</b>", "")
    ...
    this.initActionsForPrivilege(
      SessionManager.get(SessionManager.strSessionUsermanagementPrivilege)
        .privilege
    )
  ...
  popEditRoleModal(role) {
    ...
    const connection = ReactivityProvider.findOne(
      ReactivityProvider.types.Connections,
      { _id: SessionManager.get(SessionManager.strSessionConnection)._id }
    )
    const runOnAdminDB = UIComponents.Checkbox.getState(
      $("#inputRunOnAdminDBToFetchUsers")
    )
  ...
  getResourceObject(resourceString) {
    if (resourceString !== "anyResource" && resourceString !== "cluster") {
      ...
      if (resourceString.indexOf("@") !== -1) {
        ...
        result.collection = resourceString
          .substr(0, resourceString.indexOf("@"))
          .replace("<b>", "")
          .replace("</b>", "")
  ...
  initResourcesForPrivileges(dbToSelect, collectionToSelect) {
    Communicator.call({
      ...
      callback: (err, result) => {
        ...
        if (
          dbToSelect &&
          dbToSelect !== "anyResource" &&
          dbToSelect !== "cluster"
        )
          data[dbToSelect] = dbToSelect
        ...
        UIComponents.Combobox.init({
          ...
          prependOptions: $(
            "<optgroup id='optAnyResource' label='Any Resource'><option value='anyResource'>anyResource</option></optgroup>" +
  ...
  initActionsForPrivilege(actions) {
    Communicator.call({
      ...
      callback: (err, result) => {
        ...
        if (actions)
          data = Object.assign(data, Helper.populateComboboxData(actions))
  ...
  initRolesForDBForInheritRole() {
    ...
    Communicator.call({
      ...
      callback: (err, result) => {
        ...
        UIComponents.Combobox.init({
          selector: $("#cmbRolesForDBForInheritedRole"),
          data,
          options: {
            create_option: true,
            persistent_create_option: true,
        ...
        if (err || result.error) ErrorHandler.showMeteorFuncError(err, result)
          ...
          UIComponents.DataTable.setupDatatable({
            ...
            columnDefs: [
              getActionColumn({
                actionType: "VIEW",
                editClass: "editor_edit",
                targetIndex: 3,
              }),
              getActionColumn({
                deleteClass: "editor_delete_role",
                targetIndex: 4,
              }),
            ],
          })
  ...
  deleteRole() {
    ...
    const command = {
      dropRole: SessionManager.get(SessionManager.strSessionUsermanagementRole)
        .role,
    }
    UsermanagementHelper.proceedDroppingRoleOrUser(
      "#btnCloseUMRoles",
      command,
      () => {
        this.initRoles()
