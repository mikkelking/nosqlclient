...
const convertObjectToGridRow = function (obj, allKeys) {
  ...
  allKeys.forEach((key) => {
    ...
    val = `${_.escape(val)}`
    if (val.length > 100)
      html += `<td title="${quoteAttr(val)}">${val.substr(0, 97)}...</td>`
    else html += `<td>${val}</td>`
  })
...
const displayJsonEditorModal = function (sData) {
  ...
  if (modal.length === 0) {
    modal = $(
      '<div class="modal fade" id="json-editor-modal" tabindex="-1" role="dialog">' +
...
const getGridEditorHtml = function (value) {
  ...
  html += "</tr></thead><tbody>"
  value.forEach((row) => {
    html += convertObjectToGridRow(row, allKeys)
  })
  html += "</tbody></table>"
...
const gatherExtraKeysForCodeMirror = function (extraKeysToAppend = {}) {
  ...
  const autoCompleteShortcut =
    ReactivityProvider.findOne(ReactivityProvider.types.Settings)
...
const setAutoCompletionOfCodeMirror = function (autoCompleteListMethod) {
  CodeMirror.hint.javascript = (editor) => {
    ...
    const list = autoCompleteListMethod
      ? autoCompleteListMethod(editor.getValue(), curWord)
      : SessionManager.get(SessionManager.strSessionDistinctFields) || []
...
const initializeCodeMirrorFirstTime = function (
  txtAreaId,
  extraKeysToAppend,
  keepValue,
  height,
  autoCompleteListMethod,
  noResize
) {
  const codeMirror = CodeMirror.fromTextArea(
    document.getElementById(txtAreaId),
    {
      mode: "javascript",
      theme: "neat",
  ...
  if (keepValue) {
    codeMirror.on("change", () => {
      SessionManager.set(
        SessionManager.strSessionSelectorValue,
        codeMirror.getValue()
      )
    })
  }
...
UIComponents.prototype = {
  DataTable: {
    ...
    toggleDatatableRowSelection(table, row) {
      if (!row || !(row instanceof $) || !$.fn.DataTable.isDataTable(table))
        return
  ...
  Editor: {
    ...
    initializeCodeMirror({
      ...
      if (
        autoCompleteListMethod &&
        typeof autoCompleteListMethod !== "function"
      )
        return
      if (
        extraKeysToAppend &&
        (typeof extraKeysToAppend !== "object" ||
          extraKeysToAppend.constructor !== Object)
      )
        return
      ...
      if (!codeMirror) {
        codeMirror = initializeCodeMirrorFirstTime(
          txtAreaId,
          extraKeysToAppend,
          keepValue,
          height,
      ...
      const selectorValue = SessionManager.get(
        SessionManager.strSessionSelectorValue
      )
  ...
  Combobox: {
    init({
      ...
      if (comboGroupLabel) {
        selector.append(
          $(`<optgroup id="optGroup" label="${comboGroupLabel}"></optgroup>`)
        )
      ...
      if (data) {
        $.each(
          sortDataByKey ? Helper.sortObjectByKey(data) : data,
          (key, value) => {
    ...
    initializeOptionsCombobox(selector, optionEnum, sessionKey) {
      ...
      if (
        !optionEnum ||
        typeof optionEnum !== "object" ||
        optionEnum.constructor !== Object
      )
        return
    ...
    setOptionsComboboxChangeEvent(
      cmb,
      sessionKey = SessionManager.strSessionSelectedOptions
    ) {
    ...
    initializeCollectionsCombobox(selector) {
      ...
      const collectionNames = SessionManager.get(
        SessionManager.strSessionCollectionNames
      )
