...
MongoDBUser.prototype = {
  async getAllActions() {
    ...
    loadedUrl("dl[class='authaction']")
      .children("dt")
      .each(function dt() {
        result.push(loadedUrl(this).attr("id").replace("authr.", ""));
      });

    await Database.removeAsync({ type: Database.types.Actions, selector: {} });
    await Database.insertAsync({
      type: Database.types.Actions,
      document: { actionList: result },
    });
    return result;
  },
  ...
  getActionInfo({ action }) {
    ...
    return loadedUrl(`dt[id='${action}']`)
      .parent("dl[class=authaction]")
      .children("dd")
      .html();
  },
  ...
  getRoleInfo({ roleName }) {
    Logger.info({
      message: "crawl-get-role-info",
      metadataToLog: { roleName },
    });
  ...
  getResourceInfo({ resource }) {
    Logger.info({
      message: "crawl-get-resource-info",
      metadataToLog: { resource },
    });
    ...
    switch (resource) {
      ...
      case "db+collection":
        return loadedUrl(
          "div[id=specify-a-collection-of-a-database-as-resource]"
        ).html();
      ...
      case "collection":
        return loadedUrl(
          "div[id=specify-collections-across-databases-as-resource]"
        ).html();

      case "non-system":
        return loadedUrl(
          "div[id=specify-all-non-system-collections-in-all-databases]"
        ).html();
