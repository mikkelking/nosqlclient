...
QueryRender.prototype = {
  executeQuery() {
    const queryTemplate = SessionManager.get(
      SessionManager.strSessionSelectedQuery
    )
    // Since user can call here by CTRL-ENTER combination, it's a must to check URL.
    if (
      FlowRouter.current().route.path === "/browseCollection" ||
      FlowRouter.current().route.path === "/adminQueries"
    ) {
  ...
  renderAfterQueryExecution(
    ...
    if (err || result.error) ErrorHandler.showMeteorFuncError(err, result)
      ...
      if (isAdmin) this.setAdminResult(result.result)
        ...
        this.setQueryResult(result.result, queryInfo, queryParams, saveHistory)

      const collectionInfo = $("#divCollectionInfo")
      let executionTime = "0 ms"
      if (collectionInfo.find("#executionTime"))
        collectionInfo.find("#executionTime").remove()
      if (result.executionTime) executionTime = `${result.executionTime} ms`
      collectionInfo.html(`${collectionInfo.html()}<div class="row" id="executionTime"><div class="col-lg-7"><b>${Helper.translate(
        { key: "execution_time" }
      )}:</b></div>
  ...
  getActiveEditorValue() {
    ...
    if (whichIsDisplayed === "aceEditor") {
      const foundAceEditor = resultContents
        .find("div.active")
        .find("pre")
        .attr("id")
      if (foundAceEditor)
        return UIComponents.Editor.getAceEditorValue(foundAceEditor)
  ...
  initQueryHistories() {
    ...
    const connectionId = SessionManager.get(
      SessionManager.strSessionConnection
    )._id
    const selectedCollection = SessionManager.get(
      SessionManager.strSessionSelectedCollection
    )
    const queryHistories = ReactivityProvider.find(
      ReactivityProvider.types.QueryHistory,
      { connectionId, collectionName: selectedCollection },
      { sort: { date: -1 } }
    )
      ...
      args: {
        history: {
          connectionId: SessionManager.get(SessionManager.strSessionConnection)
            ._id,
          collectionName: SessionManager.get(
            SessionManager.strSessionSelectedCollection
          ),
          queryName: queryInfo,
  ...
  setQueryResult(result, queryInfo, queryParams, saveHistory) {
    const isOnlyOneTab =
      $("#divActiveJsonEditor").css("display") === "none" &&
    ...
    if (isOnlyOneTab) {
      ...
      const resultTabs = $("#resultTabs")
      const settings = ReactivityProvider.findOne(
        ReactivityProvider.types.Settings
      )
      ...
      const tabTitle = `${queryInfo} - ${SessionManager.get(
        SessionManager.strSessionSelectedCollection
      )}`
      ...
      resultTabs.append(
        $(
          `<li><a href="#tab-${tabID}" data-toggle="tab"><i class="fa fa-book"></i>${tabTitle}<button class="close" type="button" title="Close">×</button></a></li>`
        )
      )
  ...
  setAdminResult(result) {
    ...
    if ($(".jsoneditor").length === 0)
      this.jsonEditor = UIComponents.Editor.initializeJSONEditor({
        selector: "jsoneditor",
        setDivData: false,
      })
    ...
    // set ace editor
    UIComponents.Editor.setAceEditorValue({
      selector: "aceeditor",
      value: result,
    })
  ...
  setAllTabsInactive() {
    $("#resultTabContents").each(function () {
      ...
      if (otherTab.find("#divActiveJsonEditor").length !== 0) {
        ...
        otherTab
          .find("#divActiveJsonEditor")
          .attr("id", `divActiveJsonEditor-${uniqueID}`)
        otherTab
          .find("#activeJsonEditor")
          .attr("id", `activeJsonEditor-${uniqueID}`)
        otherTab
          .find("#divActiveAceEditor")
          .attr("id", `divActiveAceEditor-${uniqueID}`)
        otherTab
          .find("#activeAceEditor")
          .attr("id", `activeAceEditor-${uniqueID}`)
        otherTab
          .find("#divActiveGridEditor")
          .attr("id", `divActiveGridEditor-${uniqueID}`)
        otherTab
          .find("#activeGridEditor")
          .attr("id", `activeGridEditor-${uniqueID}`)
      }
    })
  },
  ...
  getEditor(tabID) {
    ...
    if (!tabView.data("jsoneditor")) {
      const jsonEditor = UIComponents.Editor.initializeJSONEditor({
        selector: "activeJsonEditor",
        setDivData: false,
      })
  ...
  setResultToEditors(tabID, result, queryParams, queryInfo) {
    ...
    // set ace
    UIComponents.Editor.setAceEditorValue({
      selector: "activeAceEditor",
      value: result,
    })

    // set grid editor dta
    UIComponents.Editor.setGridEditorValue({
      selector: "activeGridEditor",
      value: result,
    })
  ...
  getResultTabContent(tabID, defaultView) {
    const whichIsDisplayed =
      this.getWhichResultViewShowingWithDefault(defaultView)
    ...
    // JSON editor
    const jsonStyle =
      whichIsDisplayed === "jsonEditor" ? "" : 'style="display: none"'
    result +=
      `<div id="divActiveJsonEditor" class="form-group" ${jsonStyle}> ` +
      '<div id="activeJsonEditor" style="width: 100%;height:500px" class="col-lg-12"> ' +
      "</div> </div> "

    // ACE editor
    const aceStyle =
      whichIsDisplayed === "aceEditor" ? "" : 'style="display: none"'
    result +=
      `<div id="divActiveAceEditor" class="form-group" ${aceStyle}> ` +
      '<div class="col-lg-12"> ' +
    ...
    // GRID editor
    const gridStyle =
      whichIsDisplayed === "gridEditor" ? "" : 'style="display: none"'
    result +=
      `<div id="divActiveGridEditor" class="form-group" ${gridStyle}> ` +
  ...
  clearQueryIfAdmin() {
    $.each(Enums.ADMIN_QUERY_TYPES, (key, value) => {
      if (
        value === SessionManager.get(SessionManager.strSessionSelectedQuery)
      ) {
  ...
  renderQuery(query) {
    ...
    $("#cmbQueries")
      .val(_.invert(Enums.QUERY_TYPES)[query.queryInfo])
  ...
  init() {
    ...
    UIComponents.Combobox.init({
      selector,
      data: Enums.QUERY_TYPES,
      options: {},
      comboGroupLabel: "Collection Queries",
    })
  ...
  updateQueryResultFooter() {
    setTimeout(() => {
      ...
      const activeTabQueryInfo = activeTabText.substring(
        0,
        activeTabText.indexOf(" ")
      )

      const readOnly = this.getWhichResultViewShowing() === "gridEditor"
      if (activeTabQueryInfo === "findOne" && !readOnly)
        $("#divBrowseCollectionFooter").show()
      else if (activeTabQueryInfo === "find" && !readOnly)
  ...
  showLastTab(resultTabs, tabID) {
    ...
    $("#resultTabContents").append(
      this.getResultTabContent(
        tabID,
        settings.defaultResultView || "Jsoneditor"
      )
    )
