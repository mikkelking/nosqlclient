...
const proceedQueryExecution = function ({
  methodName,
  args = {},
  isAdmin = true,
  queryParams = {},
  ...
  if (!isAdmin) {
    Object.assign(args, {
      selectedCollection: SessionManager.get(
        SessionManager.strSessionSelectedCollection
      ),
    })
  ...
  Communicator.call({
    ...
    callback: (err, result) => {
      if (!successCallback)
        QueryRender.renderAfterQueryExecution(
          err,
          result,
          isAdmin,
          methodName,
...
const checkErrorField = function (obj, fieldName) {
  if (obj.ERROR) {
    if (!fieldName) Notification.error(obj.ERROR)
      ...
      Notification.error(`syntax-error-${fieldName}`, null, {
        error: obj.ERROR,
      })
    return false
  }
...
const getFromHistoryOrEditorString = function (
  historyParams,
  divSelector,
  historyField = "selector"
) {
  let result
...
const getFromHistoryOrEditor = function (
  historyParams,
  divSelector,
  historyField = "selector"
) {
  return ExtendedJSON.convertAndCheckJSON(
    getFromHistoryOrEditorString(historyParams, divSelector, historyField)
  )
}

const renderParams = function (queryParams) {
  Object.keys(queryParams).forEach((param) => {
    const { relatedJqueryDiv, relatedJqueryInput } =
      QueryingHelper.getRelatedDom(param)
        ...
        if (relatedJqueryDiv.data("editor")) {
          if (JSON.stringify(queryParams[param]).startsWith('"function')) {
            ...
            UIComponents.Editor.setCodeMirrorValue(
              relatedJqueryDiv,
              JSON.stringify(queryParams[param], null, 1)
            )
        } else if (relatedJqueryDiv.find("input:checkbox").length !== 0)
          UIComponents.Checkbox.toggleState(
            relatedJqueryDiv,
            queryParams[param] ? "check" : "uncheck"
          )
      } else if (relatedJqueryInput.length !== 0)
        relatedJqueryInput.val(queryParams[param])
    }, 100)
  })
}

const proceedRendering = function ({
  options,
  optionEnum,
  optionCombo,
  params,
...
const proceedUpdateQueryExecution = function (historyParams, query) {
  const options = historyParams
    ? historyParams.options
    : QueryingOptions.getOptions(Enums.UPDATE_OPTIONS)
...
const getFindModifyFinalObject = function (queryStr) {
  return {
    execute(historyParams) {
      const options = historyParams
        ? historyParams.options
        : QueryingOptions.getOptions(Enums.FINDONE_MODIFY_OPTIONS)
      const selector = getFromHistoryOrEditor(historyParams, $("#divSelector"))
      ...
      if (queryStr === "findOneAndUpdate") {
        const setObject = getFromHistoryOrEditor(
          historyParams,
          $("#divSet"),
          "set"
        )
        ...
        const replacement = getFromHistoryOrEditor(
          historyParams,
          $("#divReplacement"),
          "replacement"
        )
...
const getFindFinalObject = function (queryStr, cmbOptionsId) {
  return {
    execute(historyParams, exportFormat) {
      const cursorOptions = historyParams
        ? historyParams.cursorOptions
        : QueryingOptions.getOptions(Enums.CURSOR_OPTIONS)
      const selector = getFromHistoryOrEditor(historyParams, $("#divSelector"))
      ...
      if (exportFormat) {
        const selectedCollection = SessionManager.get(
          SessionManager.strSessionSelectedCollection
        )
        window.open(`export?format=${exportFormat}&
    selectedCollection=${selectedCollection}&selector=${JSON.stringify(
          selector
        )}&cursorOptions=${JSON.stringify(cursorOptions)}&sessionId=${
        ...
        if (queryStr === "find") {
          const executeExplain = UIComponents.Checkbox.getState(
            $("#inputExplain")
          )
...
Querying.prototype = {
  initOptions(optionEnum, showRunOnAdmin, ...excludedOptions) {
    switch (optionEnum) {
      ...
      case Enums.CREATE_INDEX_OPTIONS:
        initOptions.call(
          this,
          $("#cmbCreateIndexOptions"),
          Enums.CREATE_INDEX_OPTIONS,
          showRunOnAdmin,
          ...excludedOptions
        )
        break
      ...
      case Enums.GEO_HAYSTACK_SEARCH_OPTIONS:
        initOptions.call(
          ...
          Enums.GEO_HAYSTACK_SEARCH_OPTIONS,
          showRunOnAdmin,
          ...excludedOptions
        )
        break
      case Enums.INSERT_MANY_OPTIONS:
        initOptions.call(
          this,
          $("#cmbInsertManyOptions"),
          Enums.INSERT_MANY_OPTIONS,
          showRunOnAdmin,
          ...excludedOptions
        )
        break
      ...
      case Enums.UPDATE_OPTIONS:
        initOptions.call(
          this,
          $("#cmbUpdateManyOptions"),
          Enums.UPDATE_OPTIONS,
          showRunOnAdmin,
          ...excludedOptions
        )
        initOptions.call(
      ...
      case Enums.FINDONE_MODIFY_OPTIONS:
        initOptions.call(
          this,
          $("#cmbFindOneModifyOptions"),
          Enums.FINDONE_MODIFY_OPTIONS,
          showRunOnAdmin,
          ...excludedOptions
        )
        break
  ...
  Admin: {
    executeAddUserQuery() {
      ...
      const runOnAdminDB = UIComponents.Checkbox.getState(
        $("#inputRunOnAdminDB")
      )
      proceedQueryExecution({
        methodName: "addUser",
    ...
    executeCommandQuery() {
      const command = ExtendedJSON.convertAndCheckJSON(
        UIComponents.Editor.getCodeMirrorValue($("#divCommand"))
      )
    ...
    executeValidateCollectionQuery() {
      ...
      const options = ExtendedJSON.convertAndCheckJSON(
        UIComponents.Editor.getCodeMirrorValue($("#divOptions"))
      )
  ...
  Collection: {
    SetProfilingLevel: {
      execute(historyParams) {
        ...
        const level = historyParams
          ? historyParams.level
          : selectedLevelOption.text()
        const options = {}
    ...
    Aggregate: {
      execute(historyParams) {
        const pipeline = getFromHistoryOrEditor(
          historyParams,
          $("#divPipeline"),
          "pipeline"
        )
        const options = historyParams
          ? historyParams.options
          : QueryingOptions.getOptions(Enums.AGGREGATE_OPTIONS)
    ...
    BulkWrite: {
      execute(historyParams) {
        const operations = getFromHistoryOrEditor(
          historyParams,
          $("#divBulkWrite"),
          "bulkWrite"
        )
        const options = historyParams
          ? historyParams.options
          : QueryingOptions.getOptions(Enums.BULK_WRITE_OPTIONS)
    ...
    Count: {
      execute(historyParams) {
        const selector = getFromHistoryOrEditor(
          historyParams,
          $("#divSelector")
        )
        const options = historyParams
          ? historyParams.options
          : QueryingOptions.getOptions(Enums.COUNT_OPTIONS)
    ...
    CreateIndex: {
      execute(historyParams) {
        const options = historyParams
          ? historyParams.options
          : QueryingOptions.getOptions(Enums.CREATE_INDEX_OPTIONS)
        const fields = getFromHistoryOrEditor(
          historyParams,
          $("#divFields"),
          "fields"
        )
    ...
    Distinct: {
      execute(historyParams) {
        ...
        const fieldName = historyParams
          ? historyParams.field
          : $("#inputField").val()
        const options = historyParams
          ? historyParams.options
          : QueryingOptions.getOptions(Enums.DISTINCT_OPTIONS)
    ...
    GeoHayStackSearch: {
      execute(historyParams) {
        ...
        const options = historyParams
          ? historyParams.options
          : QueryingOptions.getOptions(Enums.GEO_HAYSTACK_SEARCH_OPTIONS)
        if (options.ERROR) {
          Notification.error(options.ERROR)
          return
        }
    ...
    Group: {
      execute(historyParams) {
        let keys = getFromHistoryOrEditorString(
          historyParams,
          $("#divKeys"),
          "keys"
        )
        const condition = getFromHistoryOrEditor(
          historyParams,
          $("#divCondition"),
          "condition"
        )
        const initial = getFromHistoryOrEditor(
          historyParams,
          $("#divInitial"),
          "initial"
        )
        const reduce = getFromHistoryOrEditorString(
          historyParams,
          $("#divReduce"),
          "reduce"
        )
        const finalize = getFromHistoryOrEditorString(
          historyParams,
          $("#divFinalize"),
          "finalize"
        )
    ...
    IndexInformation: {
      execute(historyParams) {
        const fullVal = historyParams
          ? historyParams.fullInformation
          : UIComponents.Checkbox.getState($("#inputFullInformation"))
    ...
    InsertMany: {
      execute(historyParams) {
        const docs = getFromHistoryOrEditor(
          historyParams,
          $("#divDocs"),
          "docs"
        )
        const options = historyParams
          ? historyParams.options
          : QueryingOptions.getOptions(Enums.INSERT_MANY_OPTIONS)
    ...
    MapReduce: {
      execute(historyParams) {
        const options = historyParams
          ? historyParams.options
          : QueryingOptions.getOptions(Enums.MAP_REDUCE_OPTIONS)
        const map = getFromHistoryOrEditorString(
          historyParams,
          $("#divMap"),
          "map"
        )
    ...
    Stats: {
      execute(historyParams) {
        const options = historyParams
          ? historyParams.options
          : QueryingOptions.getOptions(Enums.STATS_OPTIONS)
