...
const fillDivInputs = function (
  ...
  if (relatedJqueryDiv.data("editor")) {
    ...
    if (!val.startsWith("function")) {
      ...
      if (val.ERROR)
        result.ERROR = Helper.translate({
          key: `syntax-error-${key}`,
          options: { error: val.ERROR },
        })
      else result[key] = val
    } else result[key] = val
  } else if (relatedJqueryDiv.find("input:checkbox").length !== 0)
    result[key] = UIComponents.Checkbox.getState(relatedJqueryInput)
}
  ...
  getOptions(optionEnum) {
    ...
    Object.keys(inverted).forEach((key) => {
      if (
        $.inArray(
          inverted[key],
          SessionManager.get(SessionManager.strSessionSelectedOptions)
        ) !== -1
      ) {
        const { relatedJqueryDiv, relatedJqueryInput } =
          QueryingHelper.getRelatedDom(key)

        if (relatedJqueryDiv.length !== 0)
          fillDivInputs(relatedJqueryDiv, key, result, relatedJqueryInput)
        else if (relatedJqueryInput.length !== 0)
          fillInputs(relatedJqueryInput, result, key)
      }
    })
